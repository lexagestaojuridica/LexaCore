...
export default function BIPage() {
  ...
  const fetchData = async (orgId: string) => {
    ...
    try {
      ...
      procs.forEach((p) => { const s = p.status || "ativo"; statusMap[s] = (statusMap[s] || 0) + 1; });
      ...
      recs.forEach((r) => { const c = r.category || "Honorários"; recCatMap[c] = (recCatMap[c] || 0) + Number(r.amount); });
      const receitasPorCategoria = Object.entries(recCatMap).map(([k, v]) => ({ name: k, value: v }));

      // Despesas por categoria
      const despCatMap: Record<string, number> = {};
      desps.forEach((d) => { const c = d.category || "Operacional"; despCatMap[c] = (despCatMap[c] || 0) + Number(d.amount); });
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const monthRecs = recs.filter((r) => { const dd = new Date(r.due_date); return dd >= mStart && dd <= mEnd; });
        const monthDesps = desps.filter((r) => { const dd = new Date(r.due_date); return dd >= mStart && dd <= mEnd; });

        const receita = monthRecs.reduce((s, r) => s + Number(r.amount), 0);
        const despesa = monthDesps.reduce((s, r) => s + Number(r.amount), 0);
        const honorarios = monthRecs.filter(r => (r.category || "").toLowerCase().includes("honor")).reduce((s, r) => s + Number(r.amount), 0);
        const honorariosPagos = monthRecs.filter(r => (r.category || "").toLowerCase().includes("honor") && r.status === "pago").reduce((s, r) => s + Number(r.amount), 0);
        ...
        monthlyData.push({
          ...
          honorariosPagos,
          honorariosPendentes,
          custoFixo: monthDesps.filter(d => (d.category || "").toLowerCase().includes("fix")).reduce((s, r) => s + Number(r.amount), 0),
          custoVariavel: monthDesps.filter(d => !(d.category || "").toLowerCase().includes("fix")).reduce((s, r) => s + Number(r.amount), 0),
        });
      }
      ...
      evts.forEach((e) => { const c = e.category || "compromisso"; catMap[c] = (catMap[c] || 0) + 1; });
      ...
      for (let i = 11; i >= 0; i--) {
        ...
        const mEnd = endOfMonth(d);
        const monthLabel = format(d, "MMM", { locale: ptBR });
        const novos = procs.filter((p) => { const dd = new Date(p.created_at); return dd >= mStart && dd <= mEnd; }).length;
        const encerrados = procs.filter((p) => { if (p.status !== "encerrado") return false; const dd = new Date(p.updated_at); return dd >= mStart && dd <= mEnd; }).length;
        processosRecentes.push({ month: monthLabel, novos, encerrados });
      }

      const totalReceitas = recs.reduce((s, r) => s + Number(r.amount), 0);
      const totalReceitasPagas = recs.filter(r => r.status === "pago").reduce((s, r) => s + Number(r.amount), 0);
      const totalReceitasPendentes = recs.filter(r => r.status === "pendente").reduce((s, r) => s + Number(r.amount), 0);
      const totalReceitasAtrasadas = recs.filter(r => r.status === "pendente" && new Date(r.due_date) < today).reduce((s, r) => s + Number(r.amount), 0);
      const totalDespesas = desps.reduce((s, r) => s + Number(r.amount), 0);
      const totalDespesasPagas = desps.filter(r => r.status === "pago").reduce((s, r) => s + Number(r.amount), 0);
      const totalDespesasPendentes = desps.filter(r => r.status === "pendente").reduce((s, r) => s + Number(r.amount), 0);
      const totalDespesasAtrasadas = desps.filter(r => r.status === "pendente" && new Date(r.due_date) < today).reduce((s, r) => s + Number(r.amount), 0);

      const processosComValor = procs.filter(p => p.estimated_value && Number(p.estimated_value) > 0);
      const ticketMedioProcesso = processosComValor.length > 0 ? processosComValor.reduce((s, p) => s + Number(p.estimated_value || 0), 0) / processosComValor.length : 0;
      const valorEstimadoCarteira = procs.filter(p => p.status === "ativo").reduce((s, p) => s + Number(p.estimated_value || 0), 0);
