...
// FSD Imports aligned to minutas slice
import { useDocumentos } from "../hooks/useDocumentos";
import type { Documento } from "../types/documento";
import { getFileIcon, formatFileSize } from "../types/documento";

export default function MinutasGed() {
    const {
    ...
    return (
        <div className="space-y-6" onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop}>
            {/* ── Header removed since it will be inside Tabs ── */}
            ...
            <div className="flex items-center justify-between mb-2">
                <div>
                    <h3 className="text-lg font-bold text-foreground tracking-tight">GED Inteligente</h3>
                    <p className="text-xs text-muted-foreground">Gestão Eletrônica de Documentos na nuvem</p>
                </div>
                <Button size="sm" className="gap-1.5 shadow-sm" onClick={() => setUploadOpen(true)}>
                ...
                {isLoading ? (
                    ...
                    <div className="divide-y divide-border/50">
                        {filtered.map((doc) => {
                            ...
                            const processo = processos.find((p: any) => p.id === doc.process_id);
                            const cliente = clientes.find((c: any) => c.id === doc.client_id);
                            return (
                            ...
                            <div className="space-y-1.5">
                                ...
                                <Select value={linkProcessId} onValueChange={setLinkProcessId}>
                                    ...
                                    <SelectContent>
                                        ...
                                        {processos.map((p: any) => <SelectItem key={p.id} value={p.id}>{p.number || p.title}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                            </div>
                            <div className="space-y-1.5">
